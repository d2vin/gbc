...
contract Blog {
    ...
    function fetchPosts() public view returns (Post[] memory) {
        uint itemCount = _postIds.current(); // get the number of items in the array

        Post[] memory posts = new Post[](itemCount); // create an array of Post objects
        for (uint i = 0; i < itemCount; i++) {
            uint currentId = i + 1; // get the current id
            Post storage currentItem = idToPost[currentId]; // get the current item
            posts[i] = currentItem; // add the item to the array 
        }
        return posts;
    }
